...
then
	echo "experimental_list's len is not equal to control_lists'"
	exit 1
fi
...
mkdir ../log/meta.${today_date}
mkdir ../log/meta.${today_date}/${shitu_type}
...
do
	...
	do
		...
		sh ${program} ${shitu_type} ${experimental_list[${i}]} ${control_list[${i}]} > ../log/meta.${today_date}/${shitu_type}/task.${shitu_type}.e${experimental_list[${i}]}-c${control_list[${i}]}.log 2>&1 &
		pid_pool[${j}]=$!
		sleep 5
